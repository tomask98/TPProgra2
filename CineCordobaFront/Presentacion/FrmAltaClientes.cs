...
namespace CineCordobaFront.Presentacion
{
    public partial class FrmAltaClientes : Form
    {
        ...
        {
            string url = "https://localhost:7163/proxCliente";
        ...
        {
            string url = "https://localhost:7163/traerTipoDocumentos";
            var DataJson = await ClienteSingleton.ObtenerInstancia().GetAsync(url);
            List<TipoDoc> ldoc = JsonConvert.DeserializeObject<List<TipoDoc>>(DataJson);
        ...
        {
            string url = "https://localhost:7163/traerCiudades";
            var DataJson = await ClienteSingleton.ObtenerInstancia().GetAsync(url);
        ...
        {
            string url = "https://localhost:7163/traerBarrios" + "?id=" + id;
            var DataJson = await ClienteSingleton.ObtenerInstancia().GetAsync(url);
            List<Barrios> lbarrios = JsonConvert.DeserializeObject<List<Barrios>>(DataJson);
        ...
        private async void btnConfirmar_Click(object sender, EventArgs e)
        {
            if (validar())
            {
                ...
                cliente.Barrio = (Barrios)cboBarrio.SelectedItem;
                cliente.TipoDocId = (TipoDoc)cboTipoDoc.SelectedItem;
        ...
        private async Task<bool> crearClienteAsync(Clientes cliente)
        {

            string url = "https://localhost:7163/AltaCliente";
